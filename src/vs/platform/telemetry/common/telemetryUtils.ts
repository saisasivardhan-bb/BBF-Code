...
export function supportsTelemetry(productService: IProductService, environmentService: IEnvironmentService): boolean {
	// BBF: telemetry is permanently disabled in BBF Code. This is the single
	// chokepoint every telemetry appender is gated on, so returning false here
	// keeps the upstream call sites intact while nothing is ever collected.
	return false;
}
...
export function getTelemetryLevel(configurationService: IConfigurationService): TelemetryLevel {
	// BBF: the effective gate is supportsTelemetry(), which always returns false,
	// so no appender is ever constructed and this level is never acted on.
...
export function isInternalTelemetry(productService: IProductService, configService: IConfigurationService) {
	// BBF: never classify a machine as Microsoft-internal.
	return false;
}
