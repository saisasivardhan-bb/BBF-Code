...
// BBF: Microsoft OneCollector endpoints removed. BBF Code never sends telemetry;
// these are kept as empty strings only because the appender API exposes them.
const endpointUrl = '';
...
export abstract class AbstractOneDataSystemAppender implements ITelemetryAppender {
	...
	constructor(
		...
		// BBF: never hold a client or ingestion key, so log()/flush() below always
		// short-circuit and the 1DS SDK is never loaded or contacted.
