...
type ExtensionGalleryConfig = {
	...
	/**
	 * Whether this gallery signs every extension it serves. Self-hosted galleries
	 * generally do not; claiming otherwise makes the client reject their
	 * extensions as unsigned.
	 */
	...
	async getExtensionGalleryManifest(): Promise<IExtensionGalleryManifest | null> {
		...
		return {
			...
			capabilities: {
				...
				signing: {
					// Declared by the configured gallery rather than assumed. A gallery
					// that does not sign its extensions must say so, otherwise the client
					// warns on every extension and refuses to install them in built apps.
					allPublicRepositorySigned: extensionsGallery.allPublicRepositorySigned === true,
					allPrivateRepositorySigned: extensionsGallery.allPrivateRepositorySigned === true,
				}
			}
		};
	}
