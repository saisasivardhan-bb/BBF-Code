...
function getVersionAsset(version: IRawGalleryExtensionVersion, type: string): IGalleryExtensionAsset | null {
	...
	// BBF: honour the `source` the gallery gave for this file. Upstream ignores it
	// and rebuilds the URL from assetUri, which happens to match the Microsoft CDN
	// layout but discards anything a gallery states explicitly - including the
	// data: URIs a self-hosted gallery must use for icons, since the workbench CSP
	// only permits 'self', data:, blob: and https: images.
	const targetPlatformQuery = version.targetPlatform ? `?targetPlatform=${version.targetPlatform}` : '';
	if (result.source) {
		return { uri: result.source, fallbackUri: result.source };
	}
	return {
		uri: `${version.assetUri}/${type}${targetPlatformQuery}`,
		fallbackUri: `${version.fallbackAssetUri}/${type}${targetPlatformQuery}`
	};
}
...
export abstract class AbstractExtensionGalleryService implements IExtensionGalleryService {
	...
	async query(options: IQueryOptions, token: CancellationToken): Promise<IPager<IGalleryExtension>> {
		...
		const runQuery = async (query: Query, token: CancellationToken) => {
			...
			for (let index = 0; index < extensions.length; index++) {
				...
				const defaultChatAgentExtensionId = this.productService.defaultChatAgent?.extensionId; // BBF: may be undefined
	...
	async getExtensionsControlManifest(): Promise<IExtensionsControlManifest> {
		...
		// BBF: only deprecate the default chat agent when one is configured.
