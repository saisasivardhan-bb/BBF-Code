...
/**
 * BBF: this fork ships under its own name, and roughly ninety upstream files
 * mention the original product in a string a user can read - settings
 * descriptions, notifications, menu labels, the issue reporter. Rewriting them
 * in place would put the rebrand directly on the rebase path for every upstream
 * release. Rewriting them here, at the one function every localized string
 * passes through, keeps those files untouched.
 */
const UPSTREAM_PRODUCT_NAME_PATTERN = /Visual Studio Code|VS Code|VSCode/g;

let brandName: string | undefined;

/**
 * `nls` sits below the platform layer, so the product name is read from the
 * globals that bootstrapping sets rather than through `IProductService`. The
 * fallback covers strings localized before those globals are in place.
 */
function getBrandName(): string {
	if (brandName === undefined) {
		const globals = globalThis as {
			_VSCODE_PRODUCT_JSON?: { nameLong?: string };
			vscode?: { context?: { configuration(): { product?: { nameLong?: string } } | undefined } };
		};
		brandName = globals._VSCODE_PRODUCT_JSON?.nameLong
			?? globals.vscode?.context?.configuration()?.product?.nameLong
			?? 'BBF Code';
	}
	return brandName;
}
...
/**
 * Rewrites the upstream product name in a message template. Applied to the
 * template only, never to the interpolated arguments, so an extension or file
 * that genuinely carries the old name in its own title keeps it.
 */
/**
 * The assistant this fork ships is BBF AI Coder, not Copilot. The chat UI it
 * inherits names Copilot in roughly four hundred files, so the same chokepoint
 * handles it: rewriting there would be a four-hundred-file diff to re-resolve
 * on every upstream merge.
 *
 * Longest form first, so "GitHub Copilot" does not become
 * "GitHub BBF AI Coder".
 */
const ASSISTANT_NAME = 'BBF AI Coder';
const UPSTREAM_ASSISTANT_PATTERN = /GitHub Copilot Chat|GitHub Copilot|Copilot Chat|Copilot/g;

/**
 * Rewrites upstream product and assistant names in a message template.
 *
 * Applied to the template only, never to the interpolated arguments, so an
 * extension or file that genuinely carries one of these names in its own title
 * keeps it.
 */
function rebrand(message: string): string {
	// Most strings mention neither; skip both regexes for those.
	let result = message;
	if (result.indexOf('Code') !== -1) {
		result = result.replace(UPSTREAM_PRODUCT_NAME_PATTERN, getBrandName());
	}
	if (result.indexOf('Copilot') !== -1) {
		result = result.replace(UPSTREAM_ASSISTANT_PATTERN, ASSISTANT_NAME);
	}
	return result;
}

function _format(template: string, args: (string | number | boolean | undefined | null)[]): string {
	const message = rebrand(template);
	let result: string;
