...
export interface IProductConfiguration {
	...
	/**
	 * BBF: extension ids (publisher.name) shown in the "BBF Proprietary Extensions"
	 * section of the Extensions viewlet. Leave empty until BBF extensions are published.
	 */
	readonly bbfProprietaryExtensions?: readonly string[];

	/**
	 * BBF: base URL of the BBF proprietary extension feed (see /server). When set,
	 * BBF Code polls it for BBF extensions and keeps them up to date. This is a
	 * separate channel from `extensionsGallery`, which stays pointed at Open VSX.
	 */
	readonly bbfExtensionsServiceUrl?: string;

	/**
	 * BBF: block the workbench until a Blackbox Factories Google account signs in.
	 * A deterrent rather than an entitlement check - anyone who can run the build
	 * can modify it - so it is opt-in per build and off unless set.
	 */
	readonly bbfRequireSignIn?: boolean;
	...
	readonly extensionsGallery?: {
		...
		/** BBF: self-hosted galleries do not sign extensions; see extensionGalleryManifestService. */
