...
export class ExtHostChatAgents2 extends Disposable implements ExtHostChatAgentsShape2 {
	...
	async $setRequestTools(requestId: string, tools: UserSelectedTools) {
		...
		// A participant without `chatParticipantAdditions` receives its request
		// without a tools map (see `ChatAgentRequest.to`), so there is nothing to
		// update for it.
		if (!request?.extRequest.tools) {
			return;
		}
