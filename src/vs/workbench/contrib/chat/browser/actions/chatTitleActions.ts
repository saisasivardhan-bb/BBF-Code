	...
	registerAction2(class MarkHelpfulAction extends Action2 {
		constructor() {
			super({
				...
				// BBF Code collects no response feedback -- the vote is reported
				// through telemetry this product does not ship -- so the control is
				// never offered. The action stays registered for its keybinding
				// and command-palette callers.
				menu: [{
	...
	registerAction2(class MarkUnhelpfulAction extends Action2 {
		constructor() {
			super({
				...
				// Hidden for the same reason as the Helpful vote above.
				menu: [{
					id: MenuId.ChatMessageFooter,
					group: 'navigation',
					order: 3,
