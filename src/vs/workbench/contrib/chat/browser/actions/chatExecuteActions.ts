...
export class OpenPermissionPickerAction extends Action2 {
	...
	constructor() {
		super({
			...
			f1: false,
			precondition: ChatContextKeys.enabled,
			// BBF Code shows its own Manual/Auto mode chip in this slot (see
			// bbfChatInput.contribution.ts); the generic permission levels stay
			// reachable by command only.
			menu: {
...
export class OpenSessionTargetPickerAction extends Action2 {
	...
	constructor() {
		super({
			...
			// BBF Code runs every session locally, so the "Local" target chip is
			// never offered; the command stays registered for keybinding compatibility.
			menu: [
				{
					id: MenuId.ChatInput,
					order: 0,
...
export class OpenDelegationPickerAction extends Action2 {
	...
	constructor() {
		super({
			...
			// BBF Code has nowhere to delegate a session to, so the "Local" chip
			// this picker shows on non-empty sessions is never offered.
			menu: [
				{
					id: MenuId.ChatInputSecondary,
					order: 0.5,
