...
import { BBF_DEEP_REASONING_TOGGLE_ACTION_ID, BbfSettingPickerActionItem, BbfToggleChipActionItem, getBbfSettingPickerSpec } from './bbfSettingPickerActionItem.js';
	...
	render(container: HTMLElement, initialValue: string, widget: IChatWidget) {
		...
		this.secondaryToolbar = this._register(this.instantiationService.createInstance(MenuWorkbenchToolBar, responsivePickerContainer, MenuId.ChatInputSecondary, {
			...
			actionViewItemProvider: (action, options) => {
				...
				if (action.id === BBF_DEEP_REASONING_TOGGLE_ACTION_ID && action instanceof MenuItemAction) {
					return new BbfToggleChipActionItem(action);
				}
				const bbfPickerSpec = getBbfSettingPickerSpec(action.id);
				if (bbfPickerSpec && action instanceof MenuItemAction) {
					const createPicker = () => this.instantiationService.createInstance(BbfSettingPickerActionItem, action, bbfPickerSpec, getSecondaryPickerOptions(action.id));
