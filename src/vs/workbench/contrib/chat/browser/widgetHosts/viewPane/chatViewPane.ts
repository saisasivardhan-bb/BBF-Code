	...
	private updateSessionsControlVisibility(): { changed: boolean; visible: boolean } {
		...
		if (!this.configurationService.getValue<boolean>(ChatConfiguration.ChatViewSessionsEnabled)) {
			...
			if (this.sessionsViewerOrientation === AgentSessionsViewerOrientation.Stacked) {
				// BBF Code has no Copilot setup flow: the workbench itself is gated on
				// the Blackbox Factories sign-in, so a window that got this far is set
				// up. Upstream's `sentiment.completed || hasByokModels` clause is what
				// kept this folder's chat history hidden here, since nothing in BBF
				// ever completes Copilot's setup.
