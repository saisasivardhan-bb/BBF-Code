...
import { ExtensionsListView, EnabledExtensionsView, DisabledExtensionsView, RecommendedExtensionsView, WorkspaceRecommendedExtensionsView, ServerInstalledExtensionsView, DefaultRecommendedExtensionsView, UntrustedWorkspaceUnsupportedExtensionsView, UntrustedWorkspacePartiallySupportedExtensionsView, VirtualWorkspaceUnsupportedExtensionsView, VirtualWorkspacePartiallySupportedExtensionsView, DefaultPopularExtensionsView, BBFProprietaryExtensionsView, DeprecatedExtensionsView, SearchMarketplaceExtensionsView, RecentlyUpdatedExtensionsView, OutdatedExtensionsView, StaticQueryExtensionsView, NONE_CATEGORY, AbstractExtensionsListView } from './extensionsViews.js';
...
export class ExtensionsViewletViewsContribution extends Disposable implements IWorkbenchContribution {
	...
	private createDefaultExtensionsViewDescriptors(): IViewDescriptor[] {
		...
		/*
		 * BBF proprietary extensions view
		 * Sits after Installed / Popular / Recommended. Driven by
		 * product.json's `bbfProprietaryExtensions`.
		 */
		viewDescriptors.push({
			id: 'workbench.views.extensions.bbfProprietary',
			name: localize2('bbfProprietaryExtensions', "BBF Proprietary Extensions"),
			// No flexibleHeight: an empty flexible pane collapses to zero height, which
			// would hide the section's empty-state message.
			ctorDescriptor: new SyncDescriptor(BBFProprietaryExtensionsView, [{}]),
			// Deliberately not gated on CONTEXT_HAS_GALLERY: BBF extensions may be
			// bundled or side-loaded, so the section must still show when no gallery
			// is configured or reachable. queryByIds resolves local installs first.
