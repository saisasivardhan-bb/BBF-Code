...
import { onUnexpectedError, getErrorMessage } from '../../../../base/common/errors.js';
...
// Singletons
import { IBBFExtensionsService, BBFExtensionsService, BBFExtensionsContribution, BBF_EXTENSIONS_TOKEN_SETTING } from './bbfExtensionsService.js';
registerSingleton(IExtensionsWorkbenchService, ExtensionsWorkbenchService, InstantiationType.Eager /* Auto updates extensions */);
registerSingleton(IBBFExtensionsService, BBFExtensionsService, InstantiationType.Eager /* Polls the BBF extension feed */);
...
Registry.as<IConfigurationRegistry>(ConfigurationExtensions.Configuration)
	.registerConfiguration({
		...
		type: 'object',
		properties: {
			[BBF_EXTENSIONS_TOKEN_SETTING]: {
				type: 'string',
				default: '',
				markdownDescription: localize('bbfExtensionsToken', "Access token for the BBF proprietary extension feed. Sent as a bearer token; leave empty if the feed is unauthenticated."),
			...
			'extensions.ignoreRecommendations': {
				...
				// BBF: recommendation pop-ups are upstream promotion. Off by default;
				// workspace `.vscode/extensions.json` recommendations are still listed
				// in the Recommended view for anyone who goes looking.
				default: true,
...
registerWorkbenchContribution2(BBFExtensionsContribution.ID, BBFExtensionsContribution, WorkbenchPhase.AfterRestored);

registerAction2(class CheckForBBFExtensionUpdatesAction extends Action2 {
	constructor() {
		super({
			id: 'bbf.extensions.checkForUpdates',
			title: localize2('checkForBBFExtensionUpdates', 'Check for BBF Extension Updates'),
			category: EXTENSIONS_CATEGORY,
			f1: true
		});
	}
	async run(accessor: ServicesAccessor): Promise<void> {
		const bbfExtensionsService = accessor.get(IBBFExtensionsService);
		const notificationService = accessor.get(INotificationService);
		try {
			await bbfExtensionsService.refresh({ force: true });
			notificationService.info(localize('bbfExtensionsChecked', "BBF extensions are up to date."));
		} catch (error) {
			notificationService.error(localize('bbfExtensionsCheckFailed', "Could not reach the BBF extension feed: {0}", getErrorMessage(error)));
		}
	}
});
