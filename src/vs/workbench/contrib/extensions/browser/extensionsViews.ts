...
import { IBBFExtensionsService } from './bbfExtensionsService.js';
...
export class ExtensionsListView extends AbstractExtensionsListView<IExtension> {
	...
	/** Message shown when the view has no extensions. Subclasses may override. */
	protected getEmptyMessage(): string {
		return localize('no extensions found', "No extensions found.");
	}
		...
		if (this.bodyTemplate) {
			...
			if (this.isBodyVisible()) {
				if (message) {
					...
					this.bodyTemplate.messageBox.textContent = this.getEmptyMessage();
				}
...
/**
 * BBF: shows the extensions listed in product.json's `bbfProprietaryExtensions`.
 *
 * When no ids are configured the query is empty, and an empty query would fall
 * through to a gallery text search that lists unrelated extensions. Render an
 * empty model instead so the section's view-welcome content is shown.
 */
export class BBFProprietaryExtensionsView extends ExtensionsListView {

	@memoize
	private get bbfExtensionsService(): IBBFExtensionsService {
		return this.instantiationService.invokeFunction(accessor => accessor.get(IBBFExtensionsService));
	}

	protected override getEmptyMessage(): string {
		return localize('no bbf extensions', "No BBF proprietary extensions are available yet.");
	}

	override show(): Promise<IPagedModel<IExtension>> {
		// Poll the feed whenever this section renders, so the Extensions refresh
		// button picks up newly published extensions. Throttled in the service.
		this.bbfExtensionsService.refresh().catch(() => { /* feed may be offline */ });

		// Ids come from the BBF feed when one is configured, falling back to the
		// static product.json list, so publishing does not require a restart.
		const ids = this.bbfExtensionsService.extensionIds;
		if (!ids.length) {
			return this.showEmptyModel();
		}
		return super.show(ids.map(id => `@id:${id}`).join(' '));
	}
}
