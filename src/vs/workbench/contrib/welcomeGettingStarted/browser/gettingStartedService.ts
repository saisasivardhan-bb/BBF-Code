...
export class WalkthroughsService extends Disposable implements IWalkthroughsService {
	...
	private registerWalkthroughs() {
		...
		// BBF: the built-in product tours are an upstream marketing surface, so we
		// do not ship them. `SetupAccessibility` is kept deliberately: it is gated
		// behind screen-reader mode and is a real affordance, not a tour.
		const builtInWalkthroughs = walkthroughs.filter(category => category.id === 'SetupAccessibility');
		...
		builtInWalkthroughs.forEach(async (category, index) => {
			...
			this._registerWalkthrough({
				...
				order: builtInWalkthroughs.length - index,
