...
// BBF: the agent host hosts Copilot CLI, Claude and Codex as chat backends and
// requires a GitHub sign-in of its own. BBF AI Coder is a chat participant with
// its own Google-gated access, so registering these only offered a harness that
// refused every request with "Authentication is required to start a session".
...
// Welcome Onboarding
// BBF: Copilot/Google/Apple sign-in onboarding wizard disabled (AI out of scope).
// Its module asserts product.defaultChatAgent at import time, which BBF Code does
// not define. StartupPageRunnerContribution still needs IOnboardingService, so an
// inert implementation is registered in its place.
import './contrib/welcomeOnboarding/browser/nullOnboarding.contribution.js';

// BBF: Google sign-in entry in the Accounts menu
import './contrib/bbf/browser/bbfGoogleSignIn.contribution.js';
