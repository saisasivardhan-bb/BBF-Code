...
export const OpencodePlugin = define<HttpClient.HttpClient | EventV2.Service | Scope.Scope>({
  ...
  effect: Effect.fn(function* (ctx) {
    ...
    yield* ctx.catalog.transform((catalog) => {
      ...
      // BBF Code embeds this engine and configures it through
      // OPENCODE_CONFIG_CONTENT, which is decoded as a v1 document and so never
      // populates the v2 `providers` block this catalog reads. The two knobs the
      // product needs -- the upstream URL (its proxy) and a fixed set of
      // reasoning-effort variants -- therefore come from the environment, the
      // same channel OPENCODE_API_KEY already uses.
      const baseURL = process.env.OPENCODE_ZEN_BASE_URL
      const variants = parseVariants(process.env.OPENCODE_MODEL_VARIANTS)
      const current = catalog.provider.get(ProviderV2.ID.opencode)
      if (current && (baseURL || variants.length > 0)) {
        if (baseURL) {
          catalog.provider.update(current.provider.id, (provider) => {
            provider.api = { ...provider.api, url: baseURL }
          })
        }
        const providerPackage = current.provider.api.type === "aisdk" ? current.provider.api.package : undefined
        for (const model of current.models.values()) {
          catalog.model.update(current.provider.id, model.id, (draft) => {
            if (baseURL && draft.api.url) draft.api.url = baseURL
            // Variant bodies are written in the engine's option vocabulary
            // (`reasoningEffort`); the lowerer turns them into what the model's
            // SDK package puts on the wire, the same way remote-config variants
            // are handled above.
            const lowerer = ConfigProviderOptionsV1.get(draft.api.type === "aisdk" ? draft.api.package : providerPackage)
            for (const variant of variants) {
              if (draft.variants.some((existing) => existing.id === variant.id)) continue
              draft.variants.push({ id: ModelV2.VariantID.make(variant.id), headers: {}, body: lowerer.request({ ...variant.body }) })
            }
          })
        }
      }
