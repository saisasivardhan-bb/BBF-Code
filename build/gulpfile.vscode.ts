	...
	const task = () => {
		...
		const productJsonStream = gulp.src(['product.json'], { base: '.' })
			.pipe(jsonEditor((json: Record<string, unknown>) => {
				...
				// BBF ships BBF AI Coder, so nothing stamps Copilot versions, agent
				// SDK download URLs, or the dictation runtime into product.json.
				return json;
			}))
			.pipe(es.through(function (file) {
		...
		// BBF: Copilot CLI runtime prebuilds are not shipped (copilot extension unbundled).
